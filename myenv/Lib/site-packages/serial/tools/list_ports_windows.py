    ...
    for index in range(len(GUIDs)):
        ...
        while SetupDiEnumDeviceInfo(g_hdi, index, ctypes.byref(devinfo)):
            ...
            hkey = SetupDiOpenDevRegKey(
                ...
                KEY_READ)
            port_____name_____buffer = ctypes.create_unicode_buffer(250)
            port_____name_____length = ULONG(ctypes.sizeof(port_____name_____buffer))
            RegQueryValueEx(
                hkey,
                "PortName",
                None,
                None,
                ctypes.byref(port_____name_____buffer),
                ctypes.byref(port_____name_____length))
            RegCloseKey(hkey)

            # unfortunately does this method also include parallel ports.
            # we could check for names starting with COM or just exclude LPT
            # and hope that other "unknown" names are serial ports...
            if port_____name_____buffer.value.startswith('LPT'):
                continue
            ...
            info = list_ports_common.ListPortInfo(port_____name_____buffer.value, skip_link_detection=True)
