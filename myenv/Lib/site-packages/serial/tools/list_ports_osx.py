...
# `io_____name_____t` defined as `typedef char io_____name_____t[128];`
# in `device/device_types.h`
io_____name_____size = 128
...
def get_string_property(device_type, property):
    ...
    if CFContainer:
        ...
        if output is not None:
            ...
            buffer = ctypes.create_string_buffer(io_____name_____size);
            success = cf.CFStringGetCString(CFContainer, ctypes.byref(buffer), io_____name_____size, kCFStringEncodingUTF8)
            if success:
...
def IORegistryEntryGetName(device):
    devicename = ctypes.create_string_buffer(io_____name_____size);
...
def IOObjectGetClass(device):
    classname = ctypes.create_string_buffer(io_____name_____size)
