...
_swig_module_____name_____match = re.compile(r'\s*%module\s*(.*\(\s*package\s*=\s*"(?P<package>[\w_]+)".*\)|)\s*(?P<name>[\w_]+)',
                                     re.I).match
...
def get_swig_modulename(source):
    with open(source, 'r') as f:
        name = None
        for line in f:
            m = _swig_module_____name_____match(line)
            if m:
                name = m.group('name')
                break
    return name
...
_f2py_module_____name_____match = re.compile(r'\s*python\s*module\s*(?P<name>[\w_]+)',
                                     re.I).match
_f2py_user_module_____name_____match = re.compile(r'\s*python\s*module\s*(?P<name>[\w_]*?'
...
def get_f2py_modulename(source):
    ...
    with open(source) as f:
        for line in f:
            m = _f2py_module_____name_____match(line)
            if m:
                if _f2py_user_module_____name_____match(line): # skip *__user__* names
                    continue
                name = m.group('name')
                break
    return name
