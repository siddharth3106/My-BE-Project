...
class FCompiler(CCompiler):
    ...
    def set_command(self, key, value):
        if not key in self._executable_keys:
            raise ValueError(
                ...
                (key, self.__class__.______name______))
        if is_string(value):
        ...
        def verify_command_form(name, value):
            if value is not None and not is_sequence_of_strings(value):
                raise ValueError(
                    ...
                    (name, value, self.__class__.______name______))
    ...
    def customize(self, dist = None):
        ...
        """
        log.info('customize %s' % (self.__class__.______name______))
    ...
    def dump_properties(self):
        ...
        for l in pretty_printer.generate_help("%s instance properties:" \
                                              % (self.__class__.______name______)):
            if l[:4]=='  --':
                l = '  ' + l[4:]
            print(l)
        ...
        if is_f_file(src) and not has_f90_header(src):
            ...
            if compiler is None:
                raise DistutilsExecError('f90 not supported by %s needed for %s'\
                      % (self.__class__.______name______, src))
    ...
    def module_options(self, module_dirs, module_build_dir):
        ...
        if self.module_dir_switch is not None:
            ...
            print('XXX: Fix module_dir_switch for ', self.__class__.______name______)
        if self.module_include_switch is not None:
            for d in [module_build_dir]+module_dirs:
                options.append('%s%s' % (self.module_include_switch, d))
        else:
            print('XXX: module_dirs=%r option ignored' % (module_dirs))
            print('XXX: Fix module_include_switch for ', self.__class__.______name______)
        return options
                ...
                for alias in klass.compiler_aliases:
                    if alias in fcompiler_aliases:
                        raise ValueError("alias %r defined for both %s and %s"
                                         % (alias, klass.______name______,
                                            fcompiler_aliases[alias][1].______name______))
        ...
        try:
            ...
            if requiref90 and c.compiler_f90 is None:
                raise ValueError('%s does not support compiling f90 codes, '
                                 'skipping.' % (c.__class__.______name______))
