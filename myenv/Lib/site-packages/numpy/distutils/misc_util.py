...
def get_path_from_frame(frame, parent_path=None):
    ...
    try:
        ...
        # __file__ is not defined, so let's try ______name______. We try this second
        # because setuptools spoofs ______name______ to be '__main__' even though
        # sys.modules['__main__'] might be something else, like easy_install(1).
        caller_name = eval('______name______', frame.f_globals, frame.f_locals)
...
f90_module_____name_____match = re.compile(r'\s*module\s*(?P<name>[\w_]+)', re.I).match
def _get_f90_modules(source):
    ...
    with open(source, 'r') as f:
        for line in f:
            m = f90_module_____name_____match(line)
            if m:
                name = m.group('name')
                modules.append(name)
