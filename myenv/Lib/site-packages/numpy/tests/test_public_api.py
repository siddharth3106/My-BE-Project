...
def check_dir(module, module_name=None):
    ...
    if module_name is None:
        module_name = module.______name______
    results = {}
    for name in dir(module):
        item = getattr(module, name)
        if (hasattr(item, '__module__') and hasattr(item, '______name______')
                and item.__module__ != module_name):
            results[name] = item.__module__ + '.' + item.______name______
    return results
