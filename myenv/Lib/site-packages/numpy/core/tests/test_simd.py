...
class _Test_Utility:
    ...
    def _to_unsigned(self, vector):
        if isinstance(vector, (list, tuple)):
            ...
            sfx = vector.______name______.replace("npyv_", "")
            if sfx[0] == "b":
...
class _SIMD_ALL(_Test_Utility):
    ...
    def test_misc(self):
        ...
        if self.npyv.simd_f64:
            sfxes.append("f64")
        for sfx in sfxes:
            vec_name = getattr(self, "reinterpret_" + sfx)(vdata_a).______name______
...
for target_name, npyv in targets.items():
    ...
    for sfxes, cls in tests_registry.items():
        for sfx in sfxes:
            ...
            tcls = type(f"Test{cls.______name______}_{simd_width}_{target_name}_{sfx}", inhr, attr)
            if skip_m:
                pytest.mark.skip(reason=skip_m)(tcls)
            globals()[tcls.______name______] = tcls
