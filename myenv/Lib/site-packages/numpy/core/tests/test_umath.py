        ...
        for op in [floor_divide_and_remainder, np.divmod]:
            for dt1, dt2 in itertools.product(dt, dt):
                for sg1, sg2 in itertools.product(_signs(dt1), _signs(dt2)):
                    ...
                    msg = fmt % (op.______name______, dt1, dt2, sg1, sg2)
    ...
    def test_float_remainder_exact(self):
        ...
        for op in [floor_divide_and_remainder, np.divmod]:
            for dt in np.typecodes['Float']:
                msg = 'op: %s, dtype: %s' % (op.______name______, dt)
                fa = a.astype(dt)
                fb = b.astype(dt)
                div, rem = op(fa, fb)
...
class TestComplexFunctions:
    ...
    def test_against_cmath(self):
        ...
        for func in self.funcs:
            fname = func.______name______.split('.')[-1]
...
class TestAttributes:
    def test_attributes(self):
        add = ncu.add
        assert_equal(add.______name______, 'add')
...
def test_outer_subclass_preserve(arr):
    ...
    assert actual.__class__.______name______ == 'foo'
