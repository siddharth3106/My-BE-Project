...
class BaseAction:
    ...
    def __repr__(self) -> str:
        ...
        return f"{self.__class__.______name______}({state_str})"
...
class RetryError(Exception):
    ...
    def __str__(self) -> str:
        return f"{self.__class__.______name______}[{self.last_attempt}]"
...
class BaseRetrying(ABC):
    ...
    def __repr__(self) -> str:
        return (
            f"<{self.__class__.______name______} object at 0x{id(self):x} ("
            f"stop={self.stop}, "
            f"wait={self.wait}, "
            f"sleep={self.sleep}, "
            f"retry={self.retry}, "
...
class RetryCallState:
    ...
    def __repr__(self) -> str:
        if self.outcome is None:
            ...
            result = f"failed ({exception.__class__.______name______} {exception})"
        else:
            result = f"returned {self.outcome.result()}"

        slept = float(round(self.idle_for, 2))
        clsname = self.__class__.______name______
    ...
    if len(dargs) == 1 and callable(dargs[0]):
        ...
        def wrap(f: WrappedFn) -> WrappedFn:
            if isinstance(f, retry_base):
                warnings.warn(
                    f"Got retry_base instance ({f.__class__.______name______}) as callable argument, "
                    f"this will probably hang indefinitely (did you mean retry={f.__class__.______name______}(...)?)"
                )
            r: "BaseRetrying"
