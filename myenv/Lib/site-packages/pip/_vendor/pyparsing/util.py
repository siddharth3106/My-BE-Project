...
class __config_flags:
    ...
    def _set(cls, dname, value):
        if dname in cls._fixed_names:
            warnings.warn(
                f"{cls.______name______}.{dname} {cls._type_desc} is {str(getattr(cls, dname)).upper()}"
...
def _make_synonym_function(compat_name: str, fn: C) -> C:
    ...
    if "self" == list(inspect.signature(fn).parameters)[0]:
        ...
        def _inner(self, *args, **kwargs):
            # warnings.warn(
            #     f"Deprecated - use {fn.______name______}", DeprecationWarning, stacklevel=3
            # )
    ...
    _inner.__doc__ = f"""Deprecated - use :class:`{fn.______name______}`"""
    _inner.______name______ = compat_name
...
def replaced_by_pep8(fn: C) -> Callable[[Callable], C]:
    ...
    """
    return lambda other: _make_synonym_function(other.______name______, fn)
